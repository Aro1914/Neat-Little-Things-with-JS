...
const gameOver = () => {
	...
	const gamesLeft = gamesPlayed - totalScore
	const canStillPlay = winningScore - losingScore <= gamesLeft
	if (gamesPlayed === totalScore || !canStillPlay) {
		p1btn.disabled = true
		p2btn.disabled = true
	...
	if ((gamesPlayed === totalScore || !canStillPlay) && s1 > s2) {
		score1.classList.add('winner')
	} else if ((gamesPlayed === totalScore || !canStillPlay) && s1 < s2) {
		score2.classList.add('winner')
	} else if (gamesPlayed === totalScore || !canStillPlay) {
		scores.forEach((element) => {
